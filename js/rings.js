...
export class HealthRings {
  ...
  getSpeechContours() {
    ...
    for (const ring of this._rings) {
      ...
      contours.push({
        ...
        thickness: Math.max(ring.userData.baseTube * scale.x * 7.0, 0.28),
  ...
  update(elapsed, delta) {
    ...
    for (const ring of this._rings) {
      ...
      const pitchExpansion = this._speechEnergy * (0.02 + pitchNorm * 0.12) * (1 + idx * 0.08);
      const responseExpansion = responsePulse * (0.08 + idx * 0.018);
      ...
      ring.userData.waveStrength = this._speechEnergy * (0.18 + pitchNorm * 0.30)
        + responsePulse * (0.55 + idx * 0.05);
