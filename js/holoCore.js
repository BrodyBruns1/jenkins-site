...
function countSharedWordPrefix(leftWords = [], rightWords = []) {
  let count = 0;
  while (
    count < leftWords.length &&
    count < rightWords.length &&
    normalizeTranscriptWord(leftWords[count]) === normalizeTranscriptWord(rightWords[count])
  ) {
    count += 1;
  }
  return count;
...
export class HoloCore {
  constructor(scene) {
    ...
    this._liveRibbonLastStreamTime = 0;
  ...
  setSpeechActivity({ level = 0, speaking = false } = {}) {
    ...
    // Stop events fire frequently (speech raw=undefined) mid-utterance.
    // Do NOT reset or flush here — time-based auto-flush in update() handles it.
  }
  ...
  streamLiveTranscript(fullText = '') {
    ...
    const previousWords = this._liveRibbonLastFullText
      ? this._liveRibbonLastFullText.split(/\s+/).filter(w => w)
      : [];
    const sharedPrefix = countSharedWordPrefix(previousWords, words);

    // Only reset the word high-water mark when the transcript really starts
    // over. Shorter partials with a shared prefix are usually just STT rewrites.
    if (words.length < this._liveRibbonWordHW && sharedPrefix <= 1) {
      console.log('[holoCore] NEW UTTERANCE (words dropped %d→%d)', this._liveRibbonWordHW, words.length);
      this._liveRibbonWordHW = 0;
    }

    this._liveRibbonLastFullText = full;
    this._liveRibbonLastStreamTime = performance.now();

    // Hold back a little more on longer phrases so two-word tail rewrites
    // ("red stuff" -> "bright stars") have room to settle before we commit.
    const holdBackWords = words.length >= 6 ? 2 : 1;
    const commitUpTo = Math.max(words.length - holdBackWords, 0);
    if (commitUpTo <= this._liveRibbonWordHW) return;

    const prev = this._liveRibbonWordHW;
    this._liveRibbonWordHW = commitUpTo;
    const newWords = words.slice(prev, commitUpTo);
    const text = (prev > 0 ? ' ' : '') + newWords.join(' ');

    console.log('[holoCore] ACCEPT words=%o whw=%d held=%o full=%o bufLen=%d',
      text, this._liveRibbonWordHW, words.slice(commitUpTo).join(' '), full, this._liveRibbonBuffer.length);
  ...
  update(elapsed, _delta) {
    ...
    // Auto-flush the held-back trailing word after 2s of no new text
    if (this._liveRibbonLastFullText && this._liveRibbonLastStreamTime > 0) {
      const silence = performance.now() - this._liveRibbonLastStreamTime;
      if (silence > 2000) {
        const words = this._liveRibbonLastFullText.split(/\s+/).filter(w => w);
        if (words.length > this._liveRibbonWordHW) {
          const prev = this._liveRibbonWordHW;
          this._liveRibbonWordHW = words.length;
          const text = (prev > 0 ? ' ' : '') + words.slice(prev).join(' ');
          console.log('[holoCore] AUTO-FLUSH words=%o whw=%d', text, this._liveRibbonWordHW);
          this._bufferText(text);
        }
        this._liveRibbonLastStreamTime = 0;
      }
